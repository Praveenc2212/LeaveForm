...
export const generateEncryptedBarcode = (rollno, startDate, studentType) => {
    ...
    if (!studentType || !["HOSTELLER", "DAYSCHOLAR"].includes(studentType)) {
        throw new Error(
            "Invalid student type.  Must be 'HOSTELLER' or 'DAYSCHOLAR'"
        );
    }
...
export const validateAndDecryptBarcode = (encryptedBarcode) => {
    ...
    const diffInDays =
        (todayUTC.getTime() - generatedDate.getTime()) / (1000 * 60 * 60 * 24);

    // ✅ Valid ONLY if today or yesterday
    if (diffInDays !== 0 && diffInDays !== 1) {
        return {
            isValid: false,
            message: "Invalid barcode, Old Leave Form",
        };
    }
